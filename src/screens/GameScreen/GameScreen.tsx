...
const GameScreen = ({ route }: GameScreenProps) => {
  ...
  const { computerGuess, userNumber } = state;
  ...
  useEffect(() => {
    if (userNumber) {
      return dispatch({
        type: "GUESS",
        payload: generateRandomNum(1, 100, userNumber),
      });
    }
  }, []);

  useEffect(() => {
    setPrevComputerGuess((state) => [...state, computerGuess]);
  }, [computerGuess]);
  ...
  useEffect(() => {
    PrevComputerGuess.length > 5 && gameOver();
  }, [[PrevComputerGuess]]);
  ...
  const guess = (direction: string) => {
    if (
      ...
      Alert.alert("Liar!!", "I pitty the fool that lies", [
        { text: "My bad dwag", style: "destructive" },
      ]);
      return;
    }
    ...
    dispatch({
      type: "GUESS",
      payload: generateRandomNum(minBoudary, maxBoundary, computerGuess),
    });
  };
